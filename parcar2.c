     ...
     char *argv[];
{
  ...
  i=0; posic = 0;
  while(*ptr != ';') {
      if(*ptr == 's' || *ptr == 'n' || *ptr == 'F' ) {
          fil[i].typ = *ptr; ++ptr; fil[i].sz = atoi(ptr);
      } else fil[i].typ = *ptr;
   switch( fil[i].typ ) {
...
{
  int i,j;
  char s[4096],s2[512],*dato,*fins, anu_z[5], mes_z[3], dia_z[3];
  ...
  for ( i=0; i < tofil; i++) {
   ...
   switch( fil[i].typ ) {
     ...
     *(long*)dato += 86400;
     ...
     break;
   case 'F':
       // Modificado, es ahora un campo de Fecha en String y hay que hacer
       // las conversiones y validaciones DRBR 17-May-2003
       memcpy(s2,dato,fil[i].sz); crec = fil[i].sz;
       strncpy(anu_z, s2, 4);
       strncpy(mes_z, s2+4, 4);
       strncpy(dia_z, s2+6, 4);
       tim->tm_year = atoi(anu_z);
       tim->tm_mon = atoi(mes_z);
       tim->tm_mday = atoi(dia_z);
       fecha_z = mktime(&tim);
       if(fecha_z == -1) {
           sprintf(s2, "2003-01-01");
       } else {
           tim = localtime (fecha_z);
           sprintf(s2,"%4d-%02d-%02d",
