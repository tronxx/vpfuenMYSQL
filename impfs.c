...
/* Modificado para que p....#; llene con blancos lo que termina en nulo*/
#include <stdio.h>
#include <stdlib.h>
#include <curses.h>
#include <time.h>
...
extern WINDOW *win[];
void
impf (FILE * tof, char tipo, char *dats, int forma, char *fmt)
{
  char s[256],s1[256],*spcs = "                                          \
                                                                         \
                                                                       ";
  char *dato;
  int nc=0;
  ...
  if (tipo == 's')
  {
      ...
      memset (s1, 0, 256);
      strncpy(s1,dato,nc);
      if(nc > strlen(s1)) strncat(s1,spcs,(nc-strlen(s1)));
      if (vido)
        waddnstr(win[vwa],s1,nc);
      else
	fwrite (s1, nc, 1, tof);
      return;
  }
