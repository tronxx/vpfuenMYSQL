...
#include <ctype.h>
  ...
  HDBC hdbc;
  HSTMT hstmt;
  char tname[256],sufx[16],fmtf[40],fmtl[40],fmti[40], sql_z[1024], sql2_z[1024];
  void vSQLGet();
  void vSQLGet2();
  void vLoCase(char *);
...
void main(int argc,char *argv[])
{
 ...
 int accio = 0,i;
  if(argc == 1) {
   printf("Use: sqdefc table_name f# database user passwd -UfilarcL \n");
   exit(0);
  }
  strcpy(fmtf,"%16.2f");
  strcpy(fmtl,"%11ld");
...
void vSQLGet() /* get desc. and print them */
{
  ...
  while ( SQLDescribeCol(hstmt,
    ...
    switch(typ) {
     case 's': printf("\n\tunsigned char\t%s%s[%d]",cname,sufx,thisz);
           sprintf(buftmp_z, "%s%s,#,1,", cname, sufx); break;
    case 'd':
            printf("\n\tdouble\t%s%s",cname,sufx);
    	    sprintf(buftmp_z,"%s%s,",cname, sufx); break;
    case 'l':
            printf("\n\tlong int\t%s%s",cname,sufx);
    	    sprintf(buftmp_z,"%s%s,",cname, sufx); break;
    case 'i':
    case 'C':
            printf("\n\tint\t%s%s",cname,sufx);
    	    sprintf(buftmp_z,"%s%s,",cname, sufx); break;
    case 'f':
            printf("\n\tfloat\t%s%s",cname,sufx);
    	    sprintf(buftmp_z,"%s%s,",cname, sufx); break;
    case 'M':
           printf("\n\tunsigned char\t*%s%s",cname,sufx);
           sprintf(buftmp_z, "%s%s,#,1,", cname, sufx);
     case 'D':
           printf("\n\tDATE_STRUCT\t%s%s",cname,sufx);
    	    sprintf(buftmp_z,"%s%s,",cname, sufx); break;
     case 'T':
           printf("\n\tTIMESTAMP_STRUCT\t%s%s",cname,sufx);
    	    sprintf(buftmp_z,"%s%s,",cname, sufx); break;
     case 'H':
           printf("\n\tTIME_STRUCT\t%s%s",cname,sufx);
    	    sprintf(buftmp_z,"%s%s,",cname, sufx); break;
           break;
          break;
    }
    printf(";");
    strcat(sql2_z,buftmp_z);

    bufsz = bufsz + thisz;
    if(icol > 1) strcat(sql_z,",");
    strcat(sql_z,cname);
    icol++;
