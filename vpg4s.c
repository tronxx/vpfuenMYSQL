...
/*
   Modificado por Daniel R Basto Rivero
   Para que use MySQL
   26-Feb-2019
*/
...
{
  char *ptr, **lab, *prog;
  char s[256], *cc, finar = 0, typ, *ptr1, *ptr3, *ptr4, *ptr5, *desti, typ1;
  void *getad (), *getad1 ();
...
int aa = 0;
  ...
  fprintf(debug, "--Comienza el debugueo---\n");
#endif
   ...
   forma = valu (ptr); */
	 ...
	 whline(win[vwa], ACS_LLCORNER, 1);
		...
		case 'd':	/* device number */
                    strcpy((char *)ss, ttyname(0));
                    /* err ("td; not implemented under this OS"); */
		  break;
	        case 'u':	/* User name */
                    strcpy((char *)ss, getpwuid(getuid())->pw_name);
                    /* err ("td; not implemented under this OS"); */
		  break;
