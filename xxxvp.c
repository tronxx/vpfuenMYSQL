...
void main(int argc,char *argv[])
{
 ...
 int ts,re,i;
 if(argc==0) {
  P"VP: vpg source text pre-compiler. Use: vp program.p [+sNNNN] [+d/direc/]\n");
  exit(0);
 }
 s2[0]=0; ts=0;
 for(i=1;i<argc;i++) {
 ...
 if((src=fopen(argv[1],"r")) == NULL) end("Can't open source file");
 ...
 if((obj=fopen(s2,"w+")) == NULL)
  end("Can't open vpg object file.\n");
 ...
 fwrite(lab,sizeof(int),instr,obj);
 perror("Status");
 P"Recognized VAG code errors: %d\n",generr);
 if(iflev != 0) P"If -- then -- else... (missing dot or bar): %d\n",iflev);
 if(levl != 0) P"Loops not closed: %d\n",levl);
 /* fopen(filname,"w");  */
}
    ...
    switch(c) {
     case 'i': /* include another file */
      ...
      if((fp=fopen(sr,"r")) == NULL)
       end("Can't open Diversion File (2)\n");
     break;
...
if2:
    ...
    while((c=getc(fp)) != '(' ) putc(c,obj); putc(c,obj);
    putaval(fp);
    if((c=getc(fp)) == ',') {
  ...
  switch(c) {
   ...
   case '#': /* diversion instruction */
    ...
    if(c=='i') /* include another file */ {
     ...
     fo[nv++]=fp; sr[--i] = 0;
     if((fp=fopen(sr,"r")) == NULL) {
      end("Include File Can't be opened\n");
     }
     break;
    }
    if(c=='d') /* define a symbol to a string */ { memo = 0xff;
     ...
     if((this->stval=malloc(strlen(line)+1)) == NULL) {
      end("No more memory at %d instruction");
     }
